    ...
    <div class="wrapper">
        ...
        <div class="container">
            <form method="POST" action="add-subject.php">
                ...
                Giảng viên: <select class="form-select form-control" name="tea_id" aria-label="Default select example">
                    <?php
                    //kết nối csdl
                    include_once ("../../config/config.php");
...
if (isset($_REQUEST['id']) && isset($_REQUEST['khoa']) && isset($_REQUEST['ten']) && isset($_REQUEST['ngay_batdau']) && isset($_REQUEST['ngay_ketthuc']) && isset($_REQUEST['thoigian_hoc'])  && isset($_REQUEST['tea_id']) && isset($_REQUEST['so_tc'])) {

    //kết nối sql
    include_once ("../../config/config.php");
    //câu lệnh sql
    $query = "INSERT INTO `db_subject` (`sb_id`,`ma_khoa`,`tea_id`,`sb_ten`,`ngay_batdau`,`ngay_ketthuc`,`thoigian_hoc`,`sb_tinchi`) VALUES (?,?,?,?,?,?,?,?);";
    ...
    $giangvien = $_POST['tea_id'];
    $so_tc = $_POST['so_tc'];

    // Gán giá trị vào các tham số ẩn ('?')
    $stmt->bind_param('ssdssssd', $id, $khoa, $giangvien, $ten, $ngay_bd, $ngay_kt, $thoigian_hoc, $so_tc);
