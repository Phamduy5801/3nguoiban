...
<div class="main-content">
    <div class="wrapper">
        <h1 style="color:#a83232">Thêm thông báo khoa</h1>
        <br>
        <?php
        ...
        <div class="container">
            ...
            <form method="POST" action="update-inf-khoa.php?ma_khoa=<?php echo $id ?>">

                Mã khoa: <input class="form-control" type="text"readonly name="ma_khoa" value="<?php echo $khoa['ma_khoa'] ?>" /><br>
                Tên khoa: <input class="form-control" type="text"readonly name="ten_khoa" value="<?php echo $khoa['ten_khoa'] ?>" /><br>
                Thông báo: <input class="form-control" type="text" name="tb_khoa" value="<?php echo $khoa['tb_khoa'] ?>" /><br>
                <br>
...
if (isset($_REQUEST['ma_khoa']) && isset($_REQUEST['ten_khoa'])) {
    ...
    include_once ("config/config.php");
    //câu lệnh sql
    $query = "update db_khoa set ma_khoa = ?, ten_khoa = ?, tb_khoa = ? where ma_khoa = ?";
    //thực thi câu truy vấn
    $stmt = $conn->prepare($query);
    //khởI tạo biếN và gán dữ liệu lấy được từ trong form
    $ma_khoa = $_POST['ma_khoa'];
    $ten_khoa = $_POST['ten_khoa'];
    $tb_khoa = $_POST['tb_khoa'];
    // Gán giá trị vào các tham số ẩn ('?')
    $stmt->bind_param("ssss", $ma_khoa, $ten_khoa, $tb_khoa, $ma_khoa);
    //thực thi câu truy vấn
    $stmt->execute();
    //đóng kết nối
    $conn->close();
    //quay về trang inf-khoa.php
    header("Location: inf-khoa.php");
}
